...
class Reader:
    ...
    def parse_elimination_event(self, time):
        ...
        self.eliminations.append(Elimination(
            ...
            gun_type=gun_type,
            time=round(time/1000),         # datetime.fromtimestamp(time/1000.0),
            knocked=knocked))

    def export_to_json(self, dest, player=None):
        file = open(dest, "w")
        players_json = dict()

        memory = {"players": [], "info": []}
        for elim in self.eliminations:
            if elim.eliminator not in players_json.keys():
                players_json[elim.eliminator] = {"kill": [], "ko": []}
            if elim.eliminated not in players_json.keys():
                players_json[elim.eliminated] = {"kill": [], "ko": []}

            index = 0
            while index < len(memory["players"]) and elim.time - memory["info"][index]["time"] > 60:
                del memory["players"][index]
                del memory["info"][index]
                index += 1
        ...
        if not player:
            file.write(json.dumps(players_json, separators=(',', ':')))
        elif player in players_json.keys():
            file.write(json.dumps(players_json[player], separators=(',', ':')))
        else:
            print("Error: Player "+player+" not found.")
        file.close()
